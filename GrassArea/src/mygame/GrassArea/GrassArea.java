 ...
 * mesh creation process and is an interface between the GrassHolder (Contains all the information
 * on the grass) and GrassFactory (contains all the information to generate the grass)
 * @author Stomrage
 * @version 0.1
 * 
 */
public class GrassArea extends Geometry implements GrassObject {
    ...
    /**
     * The main constructor to generate a GrassArea over the TerrainQuad geometry. 
     * @param terrain The TerrainQuad to use to generate the GrassArea
     * @param holderSize The minimum size of the GrassArea grid splitting (Size of the @see GrassHolder)
     * @param assetManager The assetManager is needed by the @see GrassFactory to generate the grass material
    ...
    public GrassArea(TerrainQuad terrain, int holderSize, AssetManager assetManager, float grassDist) throws Exception {
        ...
        for (int x = 0; x < 2; x++) {
            for (int z = 0; z < 2; z++) {
                ...
                grassPatchs[x][z] = new GrassPatch(totalSize / 2 * x - (totalSize / 2), totalSize / 2 * z - (totalSize / 2), totalSize / 2, holderSize, totalSize);
            }
        }        
    ...
    /**
     * This function calculate the number of subdivions needed to reach the GrassHolder
    ...
    /**
     * Add a density map to the GrassArea. A density map is here to change the density of the grass given by the rgba of the texture value,
     * Each added density map give you 4 more layer to add
    ...
    /**
     * Add a layer to the GrassArea. A layer is linked to a DensityMap and a ColorChannel, you can't add the same ColorChannel to a single density map 2 times.
    ...
    /**
     * This texture is an atlas for the GrassArea, the atlas need to be X tiled if you want it to work
     *  _ _
     * |_|_|...
     * Basicly it contains all the variation for your grass area
    ...
    /**
     * Define the dissolve texture for the GrassArea, this dissolve texture will be used to make the texture dissolve in the far away
    ...
    /**
     * This method is here to create the whole GrassArea mesh. The GrassAreaControl check for close Grass and give it to this method
    ...
    public void createMesh(List<GrassBlade> grassBlades) {
        ...
        //Do that when the mesh isn't changed (Just sorting)
         ...
         IntBuffer ib = (IntBuffer) mesh.getIndexBuffer().getBuffer();
        ...
        mesh.setBuffer(VertexBuffer.Type.TexCoord, texCoordSize, tb1);
        mesh.setBuffer(VertexBuffer.Type.TexCoord2, texCoordSize, tb2);
        mesh.setBuffer(VertexBuffer.Type.TexCoord3, texCoordSize, tb3);
